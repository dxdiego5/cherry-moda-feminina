...
class CreateAndSaveProductService {
    ...
    async execute({
        product_name, code, bar_code, size, quantity, cost, price, url_img, status, category
    }) {

        // verify exists product name
        const productName = await this.productRepository.findProductName(product_name);
        if (productName) {
        ...
        if (cost >= price) {
            throw new AppError(messageProduct().ERROR.PriceProductInvalid["message"]);
        }

        // verify category exists
        const categoryExists = await this.categoryRepository.findByCategoryId(category.id);
        if (!categoryExists) {
            throw new AppError(messageProduct().ERROR.CategoryNotExist["message"]);
        }
