...
class ProductRepository implements IProductRepository {
    ...
    async findProductName(product_name: string): Promise<Product> {
        return await this.repository.findOne({ where: { product_name: product_name } });
    }

    /**
     * Create and save a new Product
     */
    async createAndSave({ product_name, code, bar_code, size, quantity, cost, price, url_img, status, category }: ICreateProductDTO): Promise<void> {
        const product = this.repository.create({ product_name, code, bar_code, size, quantity, cost, price, url_img, status, category });
