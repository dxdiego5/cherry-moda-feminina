...
class CreateAndSaveCustomerService {
    ...
    async execute({
        ...
        if (!this.dateProvider.dateIsValid(birth_date)) {
            throw new AppError(
                messageCustomer().ERROR.DateOfBirthNotValid['message'],
                401
            );
        }
        ...
        if (!validate(cpf)) {
            throw new AppError(
                messageCustomer().ERROR.cpfIvalid['message'],
                401
            );
        } else {
        ...
        const clientExists = await this.customerRepository.findByCustomerExists(
            { email, cpf }
        );
        if (clientExists.length > 0) {
            throw new AppError(
                messageCustomer().ALERT.customerExists['message'],
                401
            );
        }

        return await this.customerRepository.createAndSave({
            name,
            phone,
            email,
            cpf,
